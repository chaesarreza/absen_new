...
<div class="page-header d-flex justify-content-between align-items-center mb-4 gap-3">
    ...
    <div class="date-selector">
        <form method="GET" class="d-flex align-items-center flex-wrap gap-2 tanggal-form">
...
<div class="page-body">
    ...
    <a href="scan.php?kelas_id=<?= $info['kelas_id']; ?>&mapel_id=<?= $info['mapel_id']; ?>" 
       class="btn btn-lg btn-success mb-3 w-100">
        <i class="bi bi-qr-code-scan me-2"></i>Buka Scanner QR
    </a>

    <button id="btnSemuaHadir" class="btn btn-primary mb-4 w-100">
        <i class="bi bi-check2-all me-2"></i>Tandai Semua Hadir
    </button>
<?php endif; ?>
            <div class="card">
                        ...
                        <table class="table align-middle mobile-card-table">
                            ...
                            <?php if(!empty($siswa_list)): foreach($siswa_list as $siswa): ?>
                                <tr id="siswa-<?= $siswa['id']; ?>">
                                    ...
                                    <td class="text-center" data-label="Status Kehadiran">
                                        <?php
                                        $status = $siswa['status'];
                                    ...
                                    <td class="text-center" data-label="Aksi Absensi">
                                    ...
                                    <td class="text-center" data-label="Catatan Perilaku">
        ...
        <div class="tab-pane fade" id="penilaian" role="tabpanel">
                    ...
                    <div class="card-body">
                        ...
                        <table class="table mobile-card-table">
                            ...
                            <tbody>
                                <?php if(!empty($siswa_list)): foreach($siswa_list as $siswa): ?>
                                    <tr>
                                        <td data-label="Nama Siswa"><?= htmlspecialchars($siswa['nama_siswa']); ?></td>
                                        <td data-label="Nilai"><input type="number" name="nilai[<?= $siswa['id']; ?>]" class="form-control" min="0" max="100"></td>
                                    </tr>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <?php else: ?><table class="table table-striped table-hover mobile-card-table"><thead><tr><th>Nama Siswa</th><th>Jenis Nilai</th><th>Nilai</th></tr></thead><tbody>
                        <?php foreach ($nilai_tersimpan as $nilai): ?>
                            <tr>
                                <td data-label="Nama Siswa"><?= htmlspecialchars($nilai['nama_siswa']); ?></td>
                                <td data-label="Jenis Nilai"><?= htmlspecialchars($nilai['jenis_nilai']); ?></td>
                                <td data-label="Nilai"><?= htmlspecialchars($nilai['nilai']); ?></td>
                            </tr>
                        <?php endforeach; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <?php foreach($siswa_list as $siswa): ?>
        formData.append('siswa_ids[]', '<?= $siswa["id"]; ?>');
    <?php endforeach; ?>

    fetch('absen_semua_hadir.php', {
        method: 'POST',
        body: formData
    })
    .then(res => res.json())
